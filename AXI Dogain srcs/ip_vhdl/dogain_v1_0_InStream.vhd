...
entity dogain_v1_0_InStream is
	generic (
...
end dogain_v1_0_InStream;

architecture arch_imp of dogain_v1_0_InStream is
	...
	constant NUMBER_OF_INPUT_WORDS  : integer := NUM_DATA; --8
...
begin
	...
	axis_tready <= '1' when ((mst_exec_state = WRITE_FIFO) and (write_pointer <= NUMBER_OF_INPUT_WORDS-1) and (propagation_enable = '1')) else '0';
	...
	-- Write data to FIFO flow
    process(S_AXIS_ACLK)
    begin
        if (rising_edge (S_AXIS_ACLK)) then
            if(S_AXIS_ARESETN = '0') then
                instream_req <= '0';
                instream_captured_tdata <= std_logic_vector(to_unsigned(0,C_S_AXIS_TDATA_WIDTH));
                instream_captured_tstrb <= std_logic_vector(to_unsigned(0,C_S_AXIS_TDATA_WIDTH/8));
                instream_captured_tlast <= '0';
                instream_captured_tvalid <= '0';
            elsif (fifo_wren = '1') then
                instream_req <= '1';
                instream_captured_tdata <= S_AXIS_TDATA;
                instream_captured_tstrb <= S_AXIS_TSTRB;
                instream_captured_tlast <= S_AXIS_TLAST;
                instream_captured_tvalid    <= S_AXIS_TVALID;
          end if;
        end  if;
    end process;
